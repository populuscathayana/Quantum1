...
if (length(args) != 2) stop("Usage: plot_raman_comparison.R INPUT_CSV OUTPUT_PREFIX")
input_csv <- args[[1]]
...
stopifnot(identical(series_names, labels))
...
full <- subset(long, wavenumber >= 200 & wavenumber <= 3800)
full$region <- "全谱：200–3800 cm-1"
finger <- subset(long, wavenumber >= 350 & wavenumber <= 1800)
finger$region <- "指纹区：350–1800 cm-1"
plot_data <- rbind(full, finger)
plot_data$region <- factor(plot_data$region,
                           levels = c("全谱：200–3800 cm-1", "指纹区：350–1800 cm-1"))

guide_peaks <- c(618, 797, 1003, 1291, 1604, 1654, 3073)
...
guide_data <- subset(guide_data,
  (region == "全谱：200–3800 cm-1" & peak >= 200 & peak <= 3800) |
  (region == "指纹区：350–1800 cm-1" & peak >= 350 & peak <= 1800))

peak_labels <- subset(guide_data, series == labels[[1]])
peak_labels$label <- peak_labels$peak
peak_labels$y <- ifelse(peak_labels$peak == 1654, 0.59, 0.76)
...
panel_titles$x <- ifelse(panel_titles$region == "全谱：200–3800 cm-1", 235, 365)
panel_titles$y <- 0.95
...
p <- ggplot(plot_data, aes(wavenumber, intensity, color = series)) +
  ...
  geom_line(linewidth = 0.62, lineend = "round") +
  ...
  labs(
    title = "苯甲酸 Raman 光谱统一对比",
    subtitle = "实验谱、公开参考理论谱、两个单体构型与二聚体；各谱独立归一化",
    x = expression(paste("Raman shift / cm"^{-1})),
    y = "归一化强度",
    caption = paste0(
      "实验：SDBS RM-01-00563（488.0 nm，粉末；原图数值化）。",
      "计算谱：488.0 nm、298.15 K Raman 强度换算，Gaussian FWHM = 20 cm-1。\n",
      "单体与二聚体缩放因子 0.9613；参考理论 0.9679。",
      "归一化范围：0–4000 cm-1（部分低频峰不显示）；灰线为实验标注峰位。"
    )
  ) +
